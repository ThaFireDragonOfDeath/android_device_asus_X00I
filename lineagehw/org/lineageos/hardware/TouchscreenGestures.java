...
public class TouchscreenGestures {
    ...
    // 1 = unknown
    // 2 = Focal Touch
    private static int touchpad_variant = 0;

    private static final String[] GESTURE_PATHS = {
        "/proc/tpd_gesture", // Focal tech gesture activate path
    };

    // Id, name, keycode
    private static final TouchscreenGesture[] TOUCHSCREEN_GESTURES_FT = {
        new TouchscreenGesture(0, "Letter C", 752),
        new TouchscreenGesture(1, "Letter E", 753),
        new TouchscreenGesture(2, "Letter S", 754),
        new TouchscreenGesture(3, "Letter V", 755),
        new TouchscreenGesture(4, "Letter W", 756),
        new TouchscreenGesture(5, "Letter Z", 757),
        new TouchscreenGesture(6, "Swipe UP", 758),
        new TouchscreenGesture(7, "Double Tap", 759),
    };
    
    // Help functions
    public static int getTpVariant() {
        int currentVariant = 2;
        for (String path : GESTURE_PATHS) {
            if (FileUtils.isFileWritable(path) || FileUtils.isFileReadable(path)) {
    ...
    public static boolean isSupported() {
        if(touchpad_variant == 0) {
            touchpad_variant = getTpVariant();
        }
        
        if(touchpad_variant < 2) {
            return false;
        }
    ...
    public static TouchscreenGesture[] getAvailableGestures() {
        ...
        if(touchpad_variant == 2) {
            return TOUCHSCREEN_GESTURES_FT;
        }
     ...
     */
    public static boolean setGestureEnabled(final TouchscreenGesture gesture, final boolean state) {
        if(touchpad_variant < 2) {
            return false;
        }
        ...
        return FileUtils.writeLine(GESTURE_PATHS[touchpad_variant - 2], "1");
    }
}
